...
34. Do not assume facts, dependencies, current state, or outcomes. Trace the complete relevant path from input through every affected component, saved data, installation/package behavior, and final user-visible result before making a change. Establish the exact cause with concrete evidence; do not use speculation or "I think" as a basis for a fix. If evidence is missing, continue tracing or ask for the specific missing information. Change only the proven cause and verify the requested result. Never call a change fixed merely because it builds or passes a component check; explicitly distinguish what passed from what remains unverified. This is a standing requirement for every future task in this workspace.
