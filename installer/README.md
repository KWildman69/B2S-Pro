# B2S Pro Online Installer

`B2SProInstaller.exe` installs the B2S Pro Designer beside the original B2S
Backglass Designer and can update an existing B2S Server installation without
...
- The B2S Pro Designer is installed to its own user-selected folder.
...
- A `B2S Pro` shortcut is created on the current user's desktop and in a
  `B2S Pro` Start Menu folder. Reinstalling refreshes both shortcuts without
  changing the original Designer's shortcuts.

## Package sources

For private development, place `B2S-Latest-Complete-Build.zip` and its
`.sha256` file beside the installer. The installer will offer that package as
the local test build.

When the GitHub repository is public, the installer can query the latest
release of `KWildman69/B2S-Pro`, download `B2S-Latest-Complete-Build.zip`, and
verify it with `B2S-Latest-Complete-Build.zip.sha256` before installing.
...
`dist/B2SProInstaller.exe`

Use `dist/B2SProInstaller.SelfTest.exe --self-test <complete-build.zip>` to
validate the package reader, architecture selection, protected-file rules,
backup behavior, and sandbox installation without elevation or contact with a
real Visual Pinball installation. The self-test runner is a development output
and is not distributed to users.

## Server registration

The installer requests administrator permission when it starts and runs the
included official `B2SBackglassServerRegisterApp.exe silent` after the file
