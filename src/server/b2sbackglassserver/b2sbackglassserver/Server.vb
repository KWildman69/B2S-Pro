    ...
    Private Sub CheckLamps(ByVal lamps As Object(,))
        ...
        If lamps IsNot Nothing AndAlso IsArray(lamps) Then
            ...
            For i As Integer = 0 To lamps.GetUpperBound(0)
                ...
                B2SData.SetPivotTriggerState(B2SData.PivotLampIDs, lampid, If(lampstate > lampThreshold, 1, 0))

                If lampstate > lampThreshold AndAlso (B2SData.UsedMotionPathLampIDs.ContainsKey(lampid) OrElse B2SData.UsedMotionPathRemoveLampIDs.ContainsKey(lampid)) Then
    ...
    Private Sub CheckSolenoids(ByVal solenoids As Object(,))
        ...
        If solenoids IsNot Nothing AndAlso IsArray(solenoids) Then
            ...
            For i As Integer = 0 To solenoids.GetUpperBound(0)
                ...
                B2SData.SetPivotTriggerState(B2SData.PivotSolenoidIDs, solenoidid, solenoidstate)
                B2SData.FirePhysicsLaunchers(1, solenoidid, solenoidstate)

                If solenoidstate <> 0 AndAlso (B2SData.UsedMotionPathSolenoidIDs.ContainsKey(solenoidid) OrElse B2SData.UsedMotionPathRemoveSolenoidIDs.ContainsKey(solenoidid)) Then
            ...
            Else
                ...
                If B2SData.UsedRomMechIDs.ContainsKey(mechid) Then
                    ...
                    If rotatingPicture IsNot Nothing AndAlso rotatingPicture.NativeRotation AndAlso rotatingPicture.PictureBoxType = B2SPictureBox.ePictureBoxType.MechRotatingImage Then
                        rotatingPicture.SetNativeMechPosition(mechvalue)
                    ElseIf rotatingPicture IsNot Nothing AndAlso B2SData.RotatingImages.ContainsKey(mechid) AndAlso B2SData.RotatingImages(mechid) IsNot Nothing AndAlso B2SData.RotatingImages(mechid).Count > 0 AndAlso B2SData.RotatingImages(mechid).ContainsKey(mechvalue) Then
                        rotatingPicture.BackgroundImage = B2SData.RotatingImages(mechid)(mechvalue)
...
#Region "private non VPinMAME support"
    ...
    Private Sub MyB2SSetData(ByVal id As Integer, ByVal value As Integer)
        ...
        If B2SData.IsBackglassRunning Then
            ...
            B2SData.SetPivotTriggerState(B2SData.PivotB2SIDs, id, value)
            B2SData.FirePhysicsLaunchers(3, id, value)

            If id >= 0 AndAlso id < motionPathB2SStates.Length AndAlso
               (B2SData.UsedMotionPathB2SIDs.ContainsKey(id) OrElse B2SData.UsedMotionPathStopB2SIDs.ContainsKey(id) OrElse B2SData.UsedMotionPathResumeB2SIDs.ContainsKey(id) OrElse B2SData.UsedMotionPathRemoveB2SIDs.ContainsKey(id)) Then
                Dim previousValue As Integer = motionPathB2SStates(id)
                motionPathB2SStates(id) = value
                If previousValue = 0 AndAlso value <> 0 Then
                    If B2SData.UsedMotionPathB2SIDs.ContainsKey(id) Then
                        B2SData.StartMotionPaths(B2SData.UsedMotionPathB2SIDs(id))
                    End If
                    If B2SData.UsedMotionPathRemoveB2SIDs.ContainsKey(id) Then
                        B2SData.RemoveMotionPaths(B2SData.UsedMotionPathRemoveB2SIDs(id))
                    End If
                    If B2SData.UsedMotionPathStopB2SIDs.ContainsKey(id) Then
                        For Each picbox As B2SPictureBox In B2SData.UsedMotionPathStopB2SIDs(id)
                            If picbox IsNot Nothing Then picbox.StopMotionPath()
                        Next
                    End If
                    If B2SData.UsedMotionPathResumeB2SIDs.ContainsKey(id) Then
                        For Each picbox As B2SPictureBox In B2SData.UsedMotionPathResumeB2SIDs(id)
    ...
    Private Sub MyB2SSetData(ByVal groupname As String, ByVal value As Integer)
        ...
        If B2SData.TryMoveExternalPositionGrid(groupname, value) Then Return
        If B2SData.TryMoveExternalPivot(groupname, value) Then Return
        ...
        If Not B2SStatistics.LogStatistics Then
            ...
            If B2SSettings.StartAsEXE AndAlso Not B2SData.TestMode Then
                ...
                Try
                    ...
                    ' start new process
                    switchPulsePipeName = "B2SPhysicsSwitch_" & Diagnostics.Process.GetCurrentProcess().Id.ToString() & "_" & Guid.NewGuid().ToString("N")
                    B2SData.StartSwitchPulseBridge(switchPulsePipeName)
                    process = New Process()
                    process.StartInfo.FileName = IO.Path.Combine(exePath, exeName)
                    process.StartInfo.Arguments = """" & B2SData.TableFileName & """ """ & If(B2SData.TestMode, "1", "0") & """ """ & switchPulsePipeName & """"
                    process.Start()
