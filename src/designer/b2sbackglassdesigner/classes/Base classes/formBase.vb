Public Class formBase
    ...
    Private Sub LoadSettings()
        ...
        If XmlSettings IsNot Nothing AndAlso Not String.IsNullOrEmpty(SaveName) AndAlso XmlSettings.SelectSingleNode("B2SBackglassDesignerSettings/FormSettings/" & SaveName) IsNot Nothing Then
            ...
            End If
            ' Enhanced 3.0.13: the main designer uses WindowStateManager for
            ' monitor-aware placement. The legacy loader clamps negative X/Y
            ' coordinates to the primary display, which breaks monitors placed
            ' to the left or above it. Keep this legacy geometry path only as a
            ' fallback when no newer saved main-window state exists.
            Dim useLegacyGeometry As Boolean = Not SaveName.Equals("formDesigner", StringComparison.CurrentCultureIgnoreCase) OrElse
                                               Not WindowStateManager.HasSavedState(Me)
            If useLegacyGeometry Then
    ...
    Public Sub SaveSettings()
        ...
        If helper.CheckSaveDir(BackglassProjectsPath) Then
            ...
            Catch ex As Exception
                B2SMessageBox.Show(String.Format(My.Resources.MSG_StartupSaveError, ex.Message), AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End Try
