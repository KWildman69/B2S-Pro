...
Public Class B2STabPage
    ...
    Protected Overrides Sub OnPaintBackground(e As PaintEventArgs)
        ...
        ' A subtle technical grid gives unused workspace the black-glass depth
        ' of the approved concept without drawing over the artwork.
        Using minorGrid As New Pen(Color.FromArgb(15, 38, 190, 255), 1.0F)
            For x As Integer = 0 To ClientSize.Width Step 24
                e.Graphics.DrawLine(minorGrid, x, 0, x, ClientSize.Height)
            Next
            For y As Integer = 0 To ClientSize.Height Step 24
                e.Graphics.DrawLine(minorGrid, 0, y, ClientSize.Width, y)
            Next
        End Using
        Using majorGrid As New Pen(Color.FromArgb(18, 199, 61, 255), 1.0F)
            For x As Integer = 0 To ClientSize.Width Step 120
                e.Graphics.DrawLine(majorGrid, x, 0, x, ClientSize.Height)
            Next
            For y As Integer = 0 To ClientSize.Height Step 120
                e.Graphics.DrawLine(majorGrid, 0, y, ClientSize.Width, y)
            Next
        End Using
    End Sub
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        MyBase.OnPaint(e)
        If ClientSize.Width < 8 OrElse ClientSize.Height < 8 Then Return

        e.Graphics.SmoothingMode = SmoothingMode.AntiAlias
        Using edgeGlow As New Pen(Color.FromArgb(38, 38, 190, 255), 4.0F)
            e.Graphics.DrawRectangle(edgeGlow, 2, 2, ClientSize.Width - 5, ClientSize.Height - 5)
        End Using
        Using edgeBorder As New Pen(Color.FromArgb(150, 46, 177, 255), 1.0F)
            e.Graphics.DrawRectangle(edgeBorder, 1, 1, ClientSize.Width - 3, ClientSize.Height - 3)
        End Using

        Dim canvas As Control = ActiveCanvas()
        If canvas Is Nothing Then Return
        DrawCanvasFrame(e.Graphics, canvas.Bounds)
    End Sub

    Private Function ActiveCanvas() As Control
        If DMDPictureBox IsNot Nothing AndAlso DMDPictureBox.Visible Then Return DMDPictureBox
        If PictureBox IsNot Nothing AndAlso PictureBox.Visible Then Return PictureBox
        Return Nothing
    End Function

    Private Sub DrawCanvasFrame(graphics As Graphics, canvasBounds As Rectangle)
        If canvasBounds.Width <= 0 OrElse canvasBounds.Height <= 0 Then Return

        Dim frame As Rectangle = Rectangle.Inflate(canvasBounds, 4, 4)
        Using shadowPath As GraphicsPath = RoundedCanvasPath(Rectangle.Inflate(frame, 5, 5), 10)
            Using shadow As New Pen(Color.FromArgb(42, 0, 0, 0), 9.0F)
                graphics.DrawPath(shadow, shadowPath)
            End Using
        End Using
        Using framePath As GraphicsPath = RoundedCanvasPath(frame, 8)
            Using glow As New Pen(Color.FromArgb(68, 210, 48, 255), 6.0F)
                graphics.DrawPath(glow, framePath)
            End Using
            Using border As New Pen(Color.FromArgb(235, 46, 196, 255), 1.2F)
                graphics.DrawPath(border, framePath)
            End Using
        End Using
        DrawCanvasCorners(graphics, frame)
        DrawCanvasRulers(graphics, canvasBounds)
    End Sub

    Private Sub DrawCanvasCorners(graphics As Graphics, frame As Rectangle)
        Const corner As Integer = 15
        Using marker As New Pen(Color.FromArgb(235, 224, 64, 255), 2.0F)
            marker.StartCap = LineCap.Round
            marker.EndCap = LineCap.Round
            graphics.DrawLine(marker, frame.Left, frame.Top, frame.Left + corner, frame.Top)
            graphics.DrawLine(marker, frame.Left, frame.Top, frame.Left, frame.Top + corner)
            graphics.DrawLine(marker, frame.Right, frame.Top, frame.Right - corner, frame.Top)
            graphics.DrawLine(marker, frame.Right, frame.Top, frame.Right, frame.Top + corner)
            graphics.DrawLine(marker, frame.Left, frame.Bottom, frame.Left + corner, frame.Bottom)
            graphics.DrawLine(marker, frame.Left, frame.Bottom, frame.Left, frame.Bottom - corner)
            graphics.DrawLine(marker, frame.Right, frame.Bottom, frame.Right - corner, frame.Bottom)
            graphics.DrawLine(marker, frame.Right, frame.Bottom, frame.Right, frame.Bottom - corner)
        End Using
    End Sub

    Private Shared Function RoundedCanvasPath(bounds As Rectangle, radius As Integer) As GraphicsPath
    ...
    Private Sub DrawCanvasRulers(graphics As Graphics, canvasBounds As Rectangle)
        Dim scale As Double = Math.Max(0.05R, BackglassData.Zoom / 100.0R)
        Dim majorUnits As Integer = 100
        While majorUnits * scale < 58
            majorUnits *= 2
        End While
        Dim minorUnits As Integer = Math.Max(10, majorUnits \ 5)
        Dim labelFont As New Font("Segoe UI", 7.0F, FontStyle.Regular)

        Try
            If canvasBounds.Top >= 25 Then
                Dim baseline As Integer = canvasBounds.Top - 5
                Dim rulerBounds As New Rectangle(canvasBounds.Left, Math.Max(0, canvasBounds.Top - 24), canvasBounds.Width, 19)
                Using rulerBack As New LinearGradientBrush(rulerBounds,
                                                           Color.FromArgb(32, 22, 57),
                                                           Color.FromArgb(3, 6, 12),
                                                           LinearGradientMode.Vertical)
                    graphics.FillRectangle(rulerBack, rulerBounds)
                End Using
                Using rulerPen As New Pen(Color.FromArgb(175, 38, 205, 255), 1.0F)
                    graphics.DrawLine(rulerPen, canvasBounds.Left, baseline, canvasBounds.Right, baseline)
                    Dim sourceX As Integer = 0
                    Do
                        Dim screenX As Integer = canvasBounds.Left + CInt(Math.Round(sourceX * scale))
                        If screenX > canvasBounds.Right Then Exit Do
                        If screenX >= 0 AndAlso screenX <= ClientSize.Width Then
                            Dim major As Boolean = (sourceX Mod majorUnits = 0)
                            graphics.DrawLine(rulerPen, screenX, baseline - If(major, 8, 4), screenX, baseline)
                            If major Then
                                TextRenderer.DrawText(graphics, sourceX.ToString(), labelFont,
                                                      New Point(screenX + 2, baseline - 18),
                                                      Color.FromArgb(178, 210, 222),
            ...
            If canvasBounds.Left >= 38 Then
                Dim baseline As Integer = canvasBounds.Left - 5
                Dim rulerBounds As New Rectangle(Math.Max(0, canvasBounds.Left - 32), canvasBounds.Top, 27, canvasBounds.Height)
                Using rulerBack As New LinearGradientBrush(rulerBounds,
                                                           Color.FromArgb(21, 37, 54),
                                                           Color.FromArgb(2, 5, 10),
                                                           LinearGradientMode.Horizontal)
                    graphics.FillRectangle(rulerBack, rulerBounds)
                End Using
                Using rulerPen As New Pen(Color.FromArgb(175, 202, 67, 255), 1.0F)
                    graphics.DrawLine(rulerPen, baseline, canvasBounds.Top, baseline, canvasBounds.Bottom)
                    Dim sourceY As Integer = 0
                    Do
                        Dim screenY As Integer = canvasBounds.Top + CInt(Math.Round(sourceY * scale))
                        If screenY > canvasBounds.Bottom Then Exit Do
                        If screenY >= 0 AndAlso screenY <= ClientSize.Height Then
                            Dim major As Boolean = (sourceY Mod majorUnits = 0)
                            graphics.DrawLine(rulerPen, baseline - If(major, 8, 4), screenY, baseline, screenY)
                            If major Then
                                Dim labelBounds As New Rectangle(1, screenY - 8, Math.Max(1, baseline - 11), 16)
                                TextRenderer.DrawText(graphics, sourceY.ToString(), labelFont, labelBounds,
                                                      Color.FromArgb(178, 210, 222),
                                                      TextFormatFlags.Right Or TextFormatFlags.VerticalCenter Or TextFormatFlags.NoPadding)
                            End If
                        End If
                        sourceY += minorUnits
                    Loop
            ...
            If canvasBounds.Top >= 25 AndAlso canvasBounds.Left >= 38 Then
                Dim originBounds As New Rectangle(canvasBounds.Left - 31, canvasBounds.Top - 23, 26, 18)
                Using originBrush As New LinearGradientBrush(originBounds,
                                                              Color.FromArgb(84, 65, 196),
                                                              Color.FromArgb(12, 38, 85),
                                                              LinearGradientMode.ForwardDiagonal)
                    graphics.FillRectangle(originBrush, originBounds)
                End Using
                Using originBorder As New Pen(Color.FromArgb(210, 58, 206, 255), 1.0F)
                    graphics.DrawRectangle(originBorder, originBounds)
                End Using
            End If
        Finally
            labelFont.Dispose()
    ...
    ' Enhanced 2.3.2: the actual DMD canvas image can be selected for Edit > Cut/Delete.
    Public Property IsDMDCanvasImageSelected As Boolean
        Get
            Return DMDPictureBox IsNot Nothing AndAlso DMDPictureBox.IsCanvasImageSelected
        End Get
        Set(value As Boolean)
            If DMDPictureBox IsNot Nothing Then
                DMDPictureBox.IsCanvasImageSelected = value
    ...
    End Sub
    Private Sub PictureBox_RemoveDMDCopyArea(sender As Object, e As EventArgs)
    ...
    Private Sub PictureBox_SelectedItemMoving(sender As Object, e As Mouse.MouseMoveEventArgs)
        ' B2SPictureBox already freezes the complete non-moving layer stack at
        ' drag start and paints only the moving bulbs until mouse-up. Rebuilding
        ' illumination here on every raw mouse message defeated that cache and
        ' became progressively slower as flashers were added.
    ...
    Public Sub ShowBackglassImage()
        IsDMDCanvasImageSelected = False
    ...
    Public Property DMDImage(Optional ByVal _filename As String = "") As Image
        ...
        Set(ByVal value As Image)
            ...
            If value Is Nothing Then IsDMDCanvasImageSelected = False
            PictureBox.Visible = False
            ' refresh
            Me.Invalidate()
        End Set
    ...
    Private Sub MyNew(ByVal _name As String)
        ...
        Me.BackColor = Color.FromArgb(0, 1, 3)
        ' create picture box
        ...
        AddHandler PictureBox.RemoveDMDCopyArea, AddressOf PictureBox_RemoveDMDCopyArea
    ...
    Public Sub ReelsAndLEDs_AddScore()
        ...
        With newScore
            ...
            End If
            If String.IsNullOrEmpty(.ReelType) Then
                B2SMessageBox.Show(My.Resources.MSG_CheckB2SScoreTypes, AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            End If
    ...
    Public Sub Illumination_SetB2SID(ByVal b2sid As String)
        If SelectedBulb IsNot Nothing Then
            ...
            For Each bulb As Illumination.BulbInfo In SelectedBulbsForPropertyEdit()
                bulb.B2SID = CInt(b2sid)
            Next
            BackglassData.IsDirty = True
        End If
    End Sub
    ...
    Public Sub Illumination_SetInitialState(ByVal initialState As Integer)
        If SelectedBulb IsNot Nothing Then
            ...
            For Each bulb As Illumination.BulbInfo In SelectedBulbsForPropertyEdit()
                bulb.InitialState = initialState
            Next
            BackglassData.IsDirty = True
        End If
    End Sub
    ...
    Public Sub Illumination_SetTextFont(ByVal font As Font)
        If SelectedBulb IsNot Nothing Then
            For Each bulb As Illumination.BulbInfo In SelectedBulbsForPropertyEdit()
                bulb.FontName = font.Name
                bulb.FontSize = font.Size
                bulb.FontStyle = font.Style
                bulb.IsIlluminatedImageDirty = True
            Next
            Me.Invalidate()
            BackglassData.IsDirty = True
    ...
    Private Function SelectedBulbsForPropertyEdit() As List(Of Illumination.BulbInfo)
        Dim result As New List(Of Illumination.BulbInfo)()
        If Mouse IsNot Nothing Then
            For Each item As InfoBase In Mouse.SelectedItems
                Dim bulb As Illumination.BulbInfo = TryCast(item, Illumination.BulbInfo)
                If bulb IsNot Nothing Then result.Add(bulb)
            Next
        End If
        If result.Count = 0 AndAlso SelectedBulb IsNot Nothing Then result.Add(SelectedBulb)
        Return result
    End Function
    ...
    Public Sub Illumination_SetRomFilter(ByVal rominfo As String)
        ...
        RefreshIllumination()
    End Sub
    Public Sub Illumination_AddBulb(Optional ByVal createArtworkFlasher As Boolean = False)
        BackglassData.IsDirty = True
        ' add new bulb
        Dim newBulb As Illumination.BulbInfo = New Illumination.BulbInfo
        If createArtworkFlasher Then
        ...
        With newBulb
            ...
            ' They remain off until the generated artwork image is ready and the
            ' user assigns the desired runtime trigger/state.
        End With
        If BackglassData.IsDMDImageShown Then
        ...
        ' Enhanced 3.0.6 Stage 5: one invalidation is sufficient. The Stage 3
        ' visual signature detects the new collection item and rebuilds the
        ' composite on the next paint. Toggling illumination off/on here forced
