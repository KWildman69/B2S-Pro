...
Module moduleB2S
    ...
    Public Sub UpdateStatusBar(form As formDesigner, tab As B2STabPage)
        ...
        ' File/project dashboard. Keep the full path available as a tooltip while
        ' showing only the file name in the compact status bar.
        If tab IsNot Nothing AndAlso tab.Image IsNot Nothing AndAlso Not String.IsNullOrEmpty(tab.BackglassData.ImageFileName) Then
            Dim imagePath As String = tab.BackglassData.ImageFileName
            Dim displayName As String = IO.Path.GetFileName(imagePath)
            form.tsLabelFileInfo.Image = Nothing
            form.tsLabelFileInfo.BorderSides = ToolStripStatusLabelBorderSides.None
            form.tsLabelFileInfo.Text = "IMAGE: " & displayName
            form.tsLabelFileInfo.ToolTipText = imagePath
        Else
            form.tsLabelFileInfo.Image = Nothing
            form.tsLabelFileInfo.BorderSides = ToolStripStatusLabelBorderSides.None
            form.tsLabelFileInfo.Text = "NO IMAGE LOADED"
            form.tsLabelFileInfo.ToolTipText = String.Empty
        End If

        ' Canvas dimensions and zoom are kept together because they describe the
        ' current view rather than the file on disk.
        ...
        If tab IsNot Nothing AndAlso tab.Image IsNot Nothing Then
            ...
            If Backglass.currentData.IsDMDImageShown AndAlso tab.DMDImage IsNot Nothing Then
                width = tab.DMDImage.Width
                height = tab.DMDImage.Height
            Else
                width = tab.Image.Width
                height = tab.Image.Height
            End If
            form.tsLabelFileSize.Text = "SIZE: " & width.ToString() & " × " & height.ToString() &
                                            "   ZOOM: " & tab.BackglassData.Zoom.ToString() & "%"
        Else
            form.tsLabelFileSize.Text = "SIZE: --   ZOOM: --"
        End If
    End Sub
    ...
    Public Sub UpdateStatusBar4Mouse(form As formDesigner, tab As B2STabPage, loc As Point)
        ...
        If tab IsNot Nothing AndAlso tab.Image IsNot Nothing Then
            form.tsLabelMarker.Text = "X: " & loc.X.ToString() & "   Y: " & loc.Y.ToString()
        Else
            form.tsLabelMarker.Text = "X: --   Y: --"
        End If
    End Sub
    ...
    Public Function ExportSetImages(_selectedName As String, _selectedIndex As Integer, _xmlPath As String, _defaults As String(), _nodeName As String) As Boolean
        Try
            ...
            If _selectedIndex < _defaults.Length Then
                ...
                If success Then
                    B2SMessageBox.Show($"Successfully exported {index} images! Files saved to: {outputDirectory}", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
                    Return True
                Else
                    B2SMessageBox.Show($"No images found for: {_selectedName}.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return False
                End If
            Else
                ...
                If node Is Nothing Then
                    B2SMessageBox.Show("{_nodeName} node Not found in the XML file.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return False
                End If
                ...
                If adjustedIndex < 0 OrElse adjustedIndex >= setNodes.Count Then
                    B2SMessageBox.Show($"No data found for: {_selectedName}. Adjusted Index: {adjustedIndex}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return False
                End If
            ...
            End If
        Catch ex As Exception
            B2SMessageBox.Show($"Error exporting images: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            Return False
        End Try
    End Function
    ...
    Public Function ImportAnimations(_filePath As String) As List(Of Animation.AnimationHeader)
        ...
        For Each animationNode As Xml.XmlNode In xmlDoc.DocumentElement.SelectNodes("Animation")
            ...
            Catch ex As Exception
                ...
                B2SMessageBox.Show($"Error parsing animation: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
        Next
