...
Public Class formVPM
    ...
    Private b2sPreviewProcess As Process
    ...
    Private Sub StopAndReleaseBackglassServer()
        If b2sPreviewProcess IsNot Nothing Then
            ...
            Catch
            Finally
                b2sPreviewProcess.Dispose()
                b2sPreviewProcess = Nothing
            End Try
        End If
        ' Some server installations hand the preview window to a second EXE
        ' process. Stop every new server process created by this preview run,
        ' while leaving any server that was already running untouched.
        If b2sPreviewStartedAt <> DateTime.MaxValue Then
            For Each previewProcess As Process In Process.GetProcessesByName("B2SBackglassServerEXE")
                Try
                ...
                End Try
            Next
            b2sPreviewStartedAt = DateTime.MaxValue
            b2sPreviewExistingProcessIds.Clear()
        End If
        If b2sserver Is Nothing Then Return
        Try
            b2sserver.Stop()
        Catch
        End Try
        Try
            If Runtime.InteropServices.Marshal.IsComObject(b2sserver) Then
                Runtime.InteropServices.Marshal.FinalReleaseComObject(b2sserver)
            End If
        Catch
        Finally
            b2sserver = Nothing
    ...
    Private Function B2SServerExecutablePath() As String
        Try
            Using progIdKey As RegistryKey = Registry.ClassesRoot.OpenSubKey("B2S.Server\CLSID")
                If progIdKey Is Nothing Then Return String.Empty
                Dim clsid As String = CStr(progIdKey.GetValue(String.Empty, String.Empty))
                If String.IsNullOrEmpty(clsid) Then Return String.Empty
                Using serverKey As RegistryKey = Registry.ClassesRoot.OpenSubKey("CLSID\" & clsid & "\InprocServer32")
                    If serverKey Is Nothing Then Return String.Empty
                    Dim codeBase As String = CStr(serverKey.GetValue("CodeBase", String.Empty))
                    If String.IsNullOrEmpty(codeBase) Then Return String.Empty
                    Dim dllPath As String = New Uri(codeBase).LocalPath
                    Dim exePath As String = IO.Path.Combine(IO.Path.GetDirectoryName(dllPath), "B2SBackglassServerEXE.exe")
                    If IO.File.Exists(exePath) Then Return exePath
                End Using
            End Using
        Catch
        End Try
    ...
    Private Sub Start_Click(sender As System.Object, e As System.EventArgs) Handles btnStart.Click
        ...
        If isVPinMAMEBackglass AndAlso String.IsNullOrEmpty(cmbROMName.Text) Then
            ...
            B2SMessageBox.Show(My.Resources.MSG_SelectRomName, AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
            cmbROMName.Focus()

        ElseIf String.IsNullOrEmpty(txtVPTablesFolder.Text) OrElse Not IO.Directory.Exists(txtVPTablesFolder.Text) Then

            B2SMessageBox.Show(My.Resources.MSG_EnterValidVPTablesFolder, AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
        ...
        Else
            ...
            If Not exportSucceeded Then
                ...
                End Try
            End If
            If Not exportSucceeded Then Return
            ...
            If isVPinMAMEBackglass Then
                ...
                Dim starterPath As String = IO.Path.Combine(My.Application.Info.DirectoryPath, "B2SVPinMAMEStarter.exe")
                If Not IO.File.Exists(starterPath) Then
                    B2SMessageBox.Show("B2SVPinMAMEStarter.exe is missing from the B2S Pro folder." & Environment.NewLine &
                                       "Keep B2SVPinMAMEStarter.exe in the same folder as B2SPro.exe.",
                                       AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return
                End If
                Try
                    Using process As New Process()
                        process.StartInfo.FileName = starterPath
                        process.StartInfo.Arguments = """" & Backglass.currentData.VSName & """ """ & txtVPTablesFolder.Text & """"
                        process.Start()
                    End Using
                Catch ex As Exception
                    B2SMessageBox.Show("B2SVPinMAMEStarter.exe could not be started: " & ex.Message,
                                       AppTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
                    Return
                End Try
            ...
            Else
                ...
                ' Run the test backglass out of process. Hosting B2S Server
                ' inside this per-monitor-DPI-aware designer changes WinForms'
                ' control coordinate origin while the background image remains
                ...
                If Not String.IsNullOrEmpty(serverExe) AndAlso IO.File.Exists(previewFile) Then
                    ...
                    For Each existingProcess As Process In Process.GetProcessesByName("B2SBackglassServerEXE")
                        Try
                            b2sPreviewExistingProcessIds.Add(existingProcess.Id)
                        Finally
                            existingProcess.Dispose()
                        End Try
                    Next
                    b2sPreviewStartedAt = DateTime.Now
                    b2sPreviewProcess = New Process()
                    b2sPreviewProcess.StartInfo.FileName = serverExe
                    b2sPreviewProcess.StartInfo.WorkingDirectory = IO.Path.GetDirectoryName(previewFile)
                    b2sPreviewProcess.StartInfo.Arguments = """" & previewFile & """ 1"
                    b2sPreviewProcess.StartInfo.UseShellExecute = False
                    b2sPreviewProcess.Start()
                Else
                    ' Compatibility fallback for legacy installations without
                    ' the separate server executable.
